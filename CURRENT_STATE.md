...
- The Phase 3 playable wedge is `prompt -> external-or-fixture AI worker -> compiled object -> grace/release -> released object`; edit/remix controls should return only when they route through the AI-worker boundary.
...
- Runtime demo Settings can accept a test user's Gemini key in tab memory and use the browser to call Gemini directly, then validate/compile through the shared package before submitting the draft to SpacetimeDB.
- Runtime demo now selects browser-Gemini direct generation with `VITE_AI_CLIENT_MODE=browser-gemini`, and a live prompt with no configured key shows a HUD alert with a `Go to Settings` button instead of silently falling back to fixtures.
- Runtime demo now wires canvas click-to-select to the existing raycaster: a click (drag-discriminated) picks an object and threads the selection through the world/document render path so the selection ring shows on the clicked public object; clicking empty space clears it.
- Runtime demo selected object now exposes Move/Rotate actions (and Delete only in private destructive-edit rooms): the manual selection is threaded into `selectBackendObject`, public-object transforms apply in place via a transient edit lock, delete calls the gated `delete_object` reducer, and the first-playable/Gemini/HTTP browser smokes no longer forbid the now-intended `nudge_draft`/`rotate_draft` buttons.
...
2. Add real AI-worker edit/remix requests before reintroducing any object edit affordances in the demo UI.
...
- `pnpm --filter @3dvibegame/scene-runtime-demo typecheck`: passing as of 2026-06-01.
...
- `pnpm --filter @3dvibegame/ai-planning typecheck`: passing as of 2026-06-01.
...
- Live browser Gemini BYOK smoke: `pnpm --filter @3dvibegame/scene-runtime-demo smoke:live-browser-gemini` passing with a local `.env` key as of 2026-06-01.
...
- Live first-playable browser smoke: `pnpm --filter @3dvibegame/scene-runtime-demo smoke:live-first-playable` passing with prompt-only HUD actions as of 2026-06-01.
