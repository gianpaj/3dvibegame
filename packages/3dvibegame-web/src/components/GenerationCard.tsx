...
export function GenerationCard({
  ...
  useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (confirmingDelete && event.key === "Enter") {
  ...
  }, [confirmingDelete, object, onDelete]);
  ...
  return (
    <div className="generation-card">
      <div className="generation-card-header">
        ...
        {object && (
          <span className="generation-card-category">
            {object.builder_spec.object_category}
          </span>
        )}
      </div>
        ...
        <div className="action-buttons">
          {showMove && (
            <button
              ...
              onClick={() => onDispatch("nudge_draft")}
            >
              Move
            </button>
          )}
          {showRotate && (
            <button
              className="btn-action"
              onClick={() => onDispatch("rotate_draft")}
            >
              Rotate
            </button>
          )}
          {showScaleUp && (
            <button
              className="btn-action"
              onClick={() => onDispatch("scale_draft")}
            >
              Scale ↑
            </button>
          )}
          {showScaleDown && (
            <button
              className="btn-action"
              onClick={() => onDispatch("scale_down_draft")}
            >
              Scale ↓
            </button>
          )}
          ...
          {showRelease && (
            <button
              ...
              onClick={() => onDispatch("release_object")}
            >
